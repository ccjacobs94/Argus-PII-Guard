"""
Remediation and data loss neutralization engine for Argus PII Guard.

Provides:
- Atomic in-place file redaction with automatic pre-modification backup snapshots.
- Format-specific sanitization for plain text, code, DOCX, XLSX, and PPTX.
- 7-day automatic backup retention and single-click restoration.
- OS-level Recycle Bin / Trash integration with fallback permanent deletion.
- .argusignore whitelisting and allowed exception management.
- Write-permission detection and attribute repair.
"""
...
try:
    ...
    from .state import load_cache, load_results, load_settings, save_cache, save_results, save_settings
except (ImportError, ValueError):
    from backend.scanner import calculate_file_checksum, locate_text_pii_matches, scan_state, state_lock
    from backend.state import load_cache, load_results, load_settings, save_cache, save_results, save_settings
...
def check_write_permission(file_path: str) -> bool:
    """Checks if the given file exists and possesses write permission."""
...
def fix_file_permissions(file_path: str) -> dict[str, Any]:
    """
    Removes read-only attributes from a file to enable in-place remediation.
...
def verify_file_integrity(file_path: str, expected_checksum: Optional[str] = None) -> tuple[bool, str]:
    """
    Checks if a file exists and verifies whether its current checksum matches expected_checksum.
...
def _get_backup_dir(base_dir: str = ".") -> str:
    """Resolves and ensures the .argus_backups directory exists."""
...
def _load_backup_index(base_dir: str = ".") -> list[dict[str, Any]]:
...
def _save_backup_index(index_data: list[dict[str, Any]], base_dir: str = ".") -> None:
    index_path = _get_backup_index_path(base_dir)
...
def prune_expired_backups(max_days: int = 7, base_dir: str = ".") -> int:
    """
    Removes backups older than max_days from disk and index.
...
def create_backup(file_path: str, reason: str = "Pre-redaction backup", base_dir: str = ".") -> dict[str, Any]:
    """
...
def list_backups(base_dir: str = ".") -> list[dict[str, Any]]:
    """Returns list of backup records sorted newest to oldest."""
    return _load_backup_index(base_dir)


def restore_backup(backup_id_or_path: str, base_dir: str = ".") -> dict[str, Any]:
    """Restores the original file from a backup snapshot."""
    index = _load_backup_index(base_dir)
    target_entry = next(
        (e for e in index if e.get("id") == backup_id_or_path or e.get("backup_path") == backup_id_or_path),
        None,
    )
...
def _read_file_text_with_encoding(file_path: str) -> tuple[str, str]:
    """Attempts to read text with standard encodings, falling back to ignore errors."""
...
def _write_file_text(file_path: str, content: str, encoding: str = "utf-8") -> None:
    """Writes updated text content to file."""
    with open(file_path, "w", encoding=encoding) as f:
        f.write(content)


def _redact_plain_text(
    content: str,
    line_number: int,
    start_col: int,
    end_col: int,
    ...
    if 0 <= target_idx < len(lines):
        ...
        # Exact substring match at start_col/end_col
        if 0 <= start_col < len(line) and line[start_col:end_col] == match_text:
            lines[target_idx] = line[:start_col] + masked + line[end_col:]
        else:
            # Handle masked match_text (e.g. from secret detection: "AKIA1234...CDEF")
...
# --------------------------------------------------------------------------
# Office Document Redaction Handlers
# --------------------------------------------------------------------------

def _redact_docx_content(file_path: str, match_text: str, masked: str) -> None:
    import docx
...
def _redact_xlsx_content(file_path: str, match_text: str, masked: str) -> None:
    import openpyxl
...
def _redact_pptx_content(file_path: str, match_text: str, masked: str) -> None:
    import pptx
...
def redact_file_entity(
    ...
    Generates an automatic backup in .argus_backups/ and validates checksum.
    """
    if not os.path.exists(file_path):
    ...
    from .scanner import HEIC_EXTENSIONS, IMAGE_EXTENSIONS, PDF_EXTENSIONS
    ...
    try:
        if ext == ".docx":
            _redact_docx_content(file_path, match_text, masked)
        elif ext in (".xlsx", ".xls"):
            _redact_xlsx_content(file_path, match_text, masked)
        elif ext == ".pptx":
            _redact_pptx_content(file_path, match_text, masked)
            ...
            content, used_enc = _read_file_text_with_encoding(file_path)
            updated_content = _redact_plain_text(content, line_number, start_col, end_col, match_text, masked)
            _write_file_text(file_path, updated_content, encoding=used_enc)
...
def batch_redact_file(
    ...
    try:
        ...
        # Sort reverse by line number and column to avoid column drift
...
def _sync_state_after_remediation(file_path: str, new_checksum: str) -> None:
    ...
    if not remaining_highlights:
        ...
        results = [r for r in load_results() if r.get("file") != file_path]
        ...
        reason_msg = f"Flagged with {len(remaining_highlights)} remaining PII findings"
...
def trash_or_delete_file(file_path: str, permanent: bool = False, base_dir: str = ".") -> dict[str, Any]:
    """
...
def _remove_from_state(file_path: str) -> None:
...
def load_argusignore(base_dir: str = ".") -> list[str]:
    """Reads active rules from .argusignore file."""
...
def append_argusignore_entry(rule: str, comment: str = "", base_dir: str = ".") -> None:
...
def mark_as_safe_exception(
    ...
    """
    Whitelists a file or specific finding exception.
...
def rewrite_argusignore(exceptions: list[dict[str, Any]], base_dir: str = ".") -> None:
    """Rewrites the .argusignore file using the current list of exceptions."""
    ignore_path = os.path.join(base_dir, ARGUSIGNORE_FILENAME)
    try:
        lines = []
        for ex in exceptions:
            ...
            if file_path or match_text:
                ...
                rule = f"{norm_path} :: {match_text}" if (norm_path and match_text) else (norm_path or match_text)
...
def get_allowed_exceptions() -> list[dict[str, Any]]:
...
def remove_allowed_exception(exception_id: str, base_dir: str = ".") -> dict[str, Any]:
...
def _matches_pattern_or_mask(candidate: str, rule: str) -> bool:
    """Checks exact match or partial match when wildcards/ellipses are used."""
...
def is_file_or_match_ignored(
    ...
    settings = load_settings()
    for ex in settings.get("allowed_exceptions", []):
    ...
    for line in load_argusignore(base_dir):
        line_clean = line.replace("\\", "/").lower().strip()
        if not line_clean:
            continue
        if "::" in line_clean:
            path_part, match_part = [p.strip() for p in line_clean.split("::", 1)]
            path_matches = (
                path_part == norm_path
                or norm_path.endswith(path_part)
                or fnmatch.fnmatch(filename, path_part)
                or fnmatch.fnmatch(norm_path, path_part)
            )
            if path_matches and match_text and _matches_pattern_or_mask(match_text, match_part):
                return True
        else:
            if (
                line_clean == norm_path
                or norm_path.endswith(line_clean)
                or filename == line_clean
                or fnmatch.fnmatch(filename, line_clean)
                or fnmatch.fnmatch(norm_path, f"*{line_clean.strip('/')}*")
                or (match_text and _matches_pattern_or_mask(match_text, line_clean))
            ):
                return True
